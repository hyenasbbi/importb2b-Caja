...
function bindUI(){
  ...
  $("cashBreakdownToggle").addEventListener("click", () => {
    const panel = $("cashBreakdown");
    const opening = panel.classList.contains("is-hidden");
    panel.classList.toggle("is-hidden", !opening);
    $("cashBreakdownToggle").setAttribute("aria-expanded", String(opening));
  });
  ...
  $("settleDestinationMethod").addEventListener("change", syncSettleCashHolder);
  $("settleFundsForm").addEventListener("submit", settleFunds);

  $("receivableForm").addEventListener("submit", saveReceivable);
  $("receivablePaymentMethod").addEventListener("change", syncReceivableCashHolder);
  ...
  window.addEventListener("keydown", e => {
    if(e.key === "Escape"){
      ["movement","settlement","settleFunds","receivable","receivablePayment"].forEach(closeNamedModal);
    }
  });
}
...
async function loadMovements(){
  ...
  const { data, error } = await supabase
    .from("movements")
    .select(`id,kind,amount,currency,payment_method,category,description,occurred_at,quote_type,quote_ars,ars_equivalent,created_at,updated_at,created_by,cash_holder,edited_at,edited_by,source_type,source_id,creator:profiles!movements_created_by_fkey(full_name),editor:profiles!movements_edited_by_fkey(full_name)`)
...
function filteredMovements(){
  ...
  return state.movements.filter(m =>
    ...
    (!q || `${m.category} ${m.description || ""} ${m.created_by_name || ""} ${m.cash_holder || ""}`.toLowerCase().includes(q))
  );
}
...
function renderMovementList(container, items, showActions){
  ...
  container.innerHTML = items.map(m => {
    ...
    const holder = m.payment_method === "efectivo" ? ` · ${holderLabel(m.cash_holder)}` : "";
    ...
    const action = showActions
      ? generated
        ...
        : `<button class="mini-btn" type="button" data-edit-movement="${m.id}">EDITAR</button>`
      : "";
...
function openMovementModal(movement=null){
  ...
  if(generated){
    alert("Este movimiento fue generado por una liquidación o un cobro. Debe gestionarse desde su módulo para no desincronizar la caja.");
  ...
  $("movementCashHolder").value = movement?.cash_holder || "nahuel";
...
function syncMovementConditionalFields(){
  ...
  }else if($("movementMethod").value === "usdt"){
    $("movementMethod").value = "transferencia";
  }

  const isCash = !isUsdt && $("movementMethod").value === "efectivo";
  $("cashHolderWrap").classList.toggle("is-hidden", !isCash);
...
async function saveMovement(event){
  ...
  if(currency === "ARS" && payment_method === "efectivo") cash_holder = $("movementCashHolder").value;
  ...
  const row = {
    ...
    quote_type, quote_ars, ars_equivalent, cash_holder,
...
function openSettleFundsModal(item){
  ...
  syncSettleCashHolder();
  $("settleFundsModal").classList.remove("is-hidden");
}

function syncSettleCashHolder(){
  $("settleCashHolderWrap").classList.toggle("is-hidden", $("settleDestinationMethod").value !== "efectivo");
}
...
async function settleFunds(event){
  ...
  const holder = method === "efectivo" ? $("settleCashHolder").value : null;
  const { error } = await supabase.rpc("settle_pending_funds", {
    p_settlement_id:$("settleFundsId").value,
    p_destination_method:method,
    p_cash_holder:holder
  });
...
function openReceivablePaymentModal(item){
  ...
  syncReceivableCashHolder();
  $("receivablePaymentModal").classList.remove("is-hidden");
}

function syncReceivableCashHolder(){
  $("receivableCashHolderWrap").classList.toggle("is-hidden", $("receivablePaymentMethod").value !== "efectivo");
}
...
async function recordReceivablePayment(event){
  ...
  const holder = method === "efectivo" ? $("receivablePaymentCashHolder").value : null;
